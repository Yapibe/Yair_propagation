...
# def print_aggregated_pathway_information(args: GeneralArgs, all_pathways: dict):
#     """
...
def print_aggregated_pathway_information(args, all_pathways):
    """
    Print aggregated pathway information including P-values, trends, and significant genes
    for each pathway to a text file based on a given experiment.
    ...
    if not all_pathways:
        for trend in ['Up', 'Down']:
            file_path = path.join(args.output_dir, 'Text', f'{args.Experiment_name}_{args.pathway_file}'
                                                        f'_{args.alpha}_aggregated_{trend}.txt')
            with open(file_path, 'w') as file:
                file.write("No significant pathways found.\n")
            print(f"No significant {trend.lower()} pathways found. File written with message.")
        return

    # Separate pathways into up-regulated and down-regulated
    up_pathways = {}
    down_pathways = {}
    for pathway, conditions in all_pathways.items():
        for condition_name, condition_data in conditions.items():
            ...
            if 'Up' in trend:
                up_pathways[pathway] = conditions
            elif 'Down' in trend:
                down_pathways[pathway] = conditions

    # Function to write pathways to file
    def write_pathways_to_file(pathways, trend):
        file_path = path.join(args.output_dir, 'Text', f'{args.Experiment_name}_{args.pathway_file}'
                                                        f'_{args.alpha}_aggregated_{trend}.txt')

        pathways_p_values = [(pathway, min(condition_data['P-value'] for condition_data in conditions.values()))
                             for pathway, conditions in pathways.items()]
        pathways_sorted = sorted(pathways_p_values, key=lambda x: x[1])
        ...
        with open(file_path, 'w') as file:
            for pathway, best_p_value in pathways_sorted:
                ...
                file.write(f"Trends: {', '.join(trends)}\n")
                file.write("\n")
        print(f"Aggregated {trend.lower()} pathway information written to {file_path}")

    write_pathways_to_file(up_pathways, 'Up')
    write_pathways_to_file(down_pathways, 'Down')
...
def plot_pathways_mean_scores(args, all_pathways):
    """
    Plot mean scores of pathways across all conditions and save the plot as a PNG file.
    ...
    # Separate pathways into up-regulated and down-regulated
    up_pathways = {pathway: conditions for pathway, conditions in all_pathways.items()
                   if any('Up' in condition_data['Trend'] for condition_data in conditions.values())}
    down_pathways = {pathway: conditions for pathway, conditions in all_pathways.items()
                     if any('Down' in condition_data['Trend'] for condition_data in conditions.values())}

    def plot_and_save(pathways, trend):
        mean_scores_data = {}
        p_values_data = {}
        for pathway, conditions in pathways.items():
            for condition_name, condition_data in conditions.items():
        ...
        if data_df.empty:
            print(f"Data for plotting {trend.lower()} pathways is empty. Exiting function.")
            return
        ...
        ax.set_ylabel('Pathways', fontsize=16)
        ax.set_xlabel('Mean Scores', fontsize=16)
        ax.set_title(f'Pathway Mean Scores Across Different Conditions ({trend} Regulated)', fontsize=20)
        ...
        output_file_path = path.join(args.output_dir, 'Plots', f"{args.Experiment_name}_{args.pathway_file}"
                                                               f"_{args.alpha}_plot_{trend}.pdf")
        makedirs(path.dirname(output_file_path), exist_ok=True)
        plt.savefig(output_file_path, format='pdf', bbox_inches='tight')
        plt.close()
        print(f"Plot for {trend.lower()} regulated pathways saved to {output_file_path}")

    plot_and_save(up_pathways, 'Up')
    plot_and_save(down_pathways, 'Down')
